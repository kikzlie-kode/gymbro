...
const { logsCol } = require("../services/firestore");
...
router.get("/", async (req, res) => {
  ...
  const snap = await logsCol(req.userId).where("date", ">=", sinceStr).orderBy("date", "asc").get();
  ...
  for (const doc of snap.docs) {
    ...
    byDate[log.date].totalDurationMin += Number(log.durationMin) || 0;
  }
  ...
  res.json({ days, series, streak, totalSessions: snap.size });
});
